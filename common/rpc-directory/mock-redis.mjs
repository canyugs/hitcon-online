...
class MockRedis {
  ...
  createClient() {
    const newClient = new MockRedisClient(this);
...
class MockRedisClient {
  ...
  hget(kf, cb) {
    ...
    const key = kf[0]; const field = kf[1];
  ...
  hset(kfv, cb) {
    ...
    const key = kfv[0]; const field = kfv[1]; const value = kfv[2];
  ...
  hgetall(key, cb) {
    const obj = {};
    if (this._data.has(key) && (this._data.get(key) instanceof Map)) {
      this._data.get(key).forEach((v, k) => {
        obj[k] = v;
      });
    }
